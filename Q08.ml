(* Questão 8 - Leonardo Augusto Picanço Barreto e Daniel Lopes Zordan Filho *)
fun somar(nil) = 0
...
let
	...
	fun listar(nil) = nil
	| listar(cab::cau) = if(pegaSoma(pegarPontas(novoEstado((e1, e2, e3, e4), cab))) mod 5 = 0) then cab:: listar(cau) else listar(cau)

in
	listar(possiveis)
	(* pegaSoma(g(possiveis)) *)
	(* g(possiveis) *)
end; 

(* Caso de teste *)
pedrasPont([(6,6)],[(1,6)], [(4,5),(5,6)],[(3,6)], [(4,0)]); 